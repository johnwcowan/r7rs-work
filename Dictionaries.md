...
key-value pairs: alists and hash tables, and not all hash table implementations conform
the the R6RS standard interface.  Now, however, the number of such mechanisms is
...
from SRFI 167.  In addition, plists (property lists, which alternate
symbol keys with values) have been around since the beginning of Lisp,
but there has never been any support for them in a Scheme standard.

It's inconvenient for users if other SRFIs or libraries have to
insist on accepting only one kind of dictionary.
By using the procedures of this SRFI, any procedure can take a dictionary
...
The dictionary *dictionary* is searched for an association with *key*.
