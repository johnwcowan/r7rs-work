...
It is important to allow multiple values that can result in multiple
definitions and assignments, but it is not apparent to me why we need
a special data type to implement them.
...
WG2 should consider extending the DEFINE and LET syntax to include a
nice pattern destructuring language.  But don't accept some partially
cooked kludge.
...
Do not introduce anything "not necessarily of very high quality" into
the language!  Don't do anything that Knuth and Kahan would not
approve of!  If you have good integers and assignment a user can make
his own, so this is not essential.
...
The srfi-38 stuff is certainly valuable, but it really is not
essential in the core of the language.  This is really a
common-subexpression elimination of data objects.
...
You mean "ellipsis" not "elllipse"!  
An ellipse is a shape, an ellipsis is a punctuation mark.
...
This is a default only.  The reader syntax should be 
allowed to be specified as metadata on the source of the
input.  For example, the PLT reader must be specified 
at the head of a file.  However, for compatability and 
convenience the traditional default for LISP-based systems 
should be fold-case.
...
Actually, I would like [] to be reader syntax for vector data
and allow the lambda, cond, let to accept that kind of data
as legitimate syntax for lists.  Indeed, {} should be allowed 
to represent sets.
...
Must include weak structures and ephemerons, because
these structures cannot be built with user code.
